...
enrichment_variant <- function(data_aggregated , group , variants,multiplicateur){
  ...
  for (week in semaine) {
    ...
    pourcentage_wanted <- full_desaggregated_week %>% group_by(country_code,year_week,age_group,variant) %>% summarise(nb = n()) %>%
      group_by(country_code,year_week,age_group) %>% mutate(pourcentage = nb/sum(nb))%>% filter(variant==variants & age_group==group)
    ...
    pourcentage_other <- fullother %>% group_by(country_code,year_week,age_group,variant) %>% summarise(nb = n()) %>%
      group_by(country_code,year_week) %>% mutate(pourcentage = nb/sum(nb))%>% filter(variant==variants)
    ...
    while (pourcentage_wanted$pourcentage < sum(pourcentage_other$pourcentage)*multiplicateur | pourcentage_wanted$pourcentage==1 |sum(pourcentage_other$pourcentage) == 0) {
      ...
      row_wanted <- targetted_category_wanted%>%group_by(country_code,year_week)%>% slice(1)
      random = sample(1:length(targetted_category_unwanted$country_code),1)
      variant_delete <- targetted_category_unwanted%>%group_by(country_code,year_week) %>% slice(random)
      targetted_category_unwanted <- targetted_category_unwanted%>%group_by(country_code,year_week) %>% slice(-random)

      random = sample(1:length(other_category_wanted$country_code),1)
      group_delete = other_category_wanted %>%group_by(country_code,year_week) %>%slice(random)
      other_category_wanted <- other_category_wanted %>%group_by(country_code,year_week) %>%slice(-random)
      row_unwanted = tibble(country_code = country,year_week = week,age_group=group_delete$age_group,variant=variant_delete$variant)
