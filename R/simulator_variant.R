...
simulator_variant <- function(trainset , testset,start = "2021-01",end = "2022-01",list_country_code){
  ...
  for (country in list_country_code) {
    ...
    # Data test
    test_country <- testset %>% filter(country_code == country)
    test_country$year_week_num <- as.numeric(factor(test_country$year_week,levels = levels(factor(train_country$year_week))))
    test_country$year_week_jiter <- jitter(test_country$year_week_num,factor = 0.1)

    # Prediction with KNN model
    pr <- knn(train = train_country$year_week_jiter,test = data.frame(year_week = test_country$year_week_jiter),cl=train_country$variant,k=1)
