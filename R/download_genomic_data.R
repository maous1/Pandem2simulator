...
download_genomic_data <- function(ecdcdata, variant_colname,path_dataformat,path_dataset,path_nextclade) {
  ...
  genomic_data <- genomic_data %>% select(Isolate.Collection.date, Nextclade_pango, substitutions, deletions, insertions, missing) %>% dplyr::rename(collection_date=Isolate.Collection.date) %>% dplyr::rename(variant=Nextclade_pango)
