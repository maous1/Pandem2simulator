...
simulator_combine <- function(trainset, testset,time ,geolocalisation, outcome, count= NULL,factor){
  ...
  for (i in 1:(length(date)-1)) {
    trainset_frag <- trainset %>% filter(as.Date(time)>=as.Date(date[i]) & as.Date(time)<=as.Date(date[i+1]))
    testset_frag <- testset %>% filter(as.Date(time)>=as.Date(date[i]) & as.Date(time)<=as.Date(date[i+1]))
